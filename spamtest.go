...
func SendMail(addr string, from string, to []string, msg []byte) os.Error {
	c, err := Dial(addr)
	if err != nil {
		return err
	}
...
func main(){
	...
	err := SendMail("mail2.bangkokranch.co.th:25",
			"anchalee@bangkokranch.co.th",
			[]string{"anchalee@bangkokranch.co.th"},
			[]byte("asdfasdfasdfasdfasfsadf"), 
			)
	 if err != nil {
                    //    log.Fatal(err)
                        }
